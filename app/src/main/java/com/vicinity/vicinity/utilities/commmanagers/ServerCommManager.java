...
public class ServerCommManager {
        ...
        new AsyncTask<Void, Void, Boolean>(){
            ...
            protected Boolean doInBackground(Void... params) {
                ...
                try{
                    ...
                    Log.e("DEBUG", "========================               Status code from Server in onSendCode() : " + con.getResponseCode() + "            ========================");
            ...
            protected void onPostExecute(Boolean verifySuccess) {
                if (verifySuccess){
                    ...
                    ((Registrator) context).resetCodeForm();
                }
            }
    ...
    public void onPostRegisterBusiness(final Context context, final String accId, final String placeId, final String placeLocalPhone, final String placeName, final String placeAddress){
        new AsyncTask<Void, Void, Boolean>() {
            ...
            @Override
            protected void onPostExecute(Boolean isResultSuccess) {
                if (isResultSuccess){
                    Toast.makeText(context, "Request for registering '" + placeName + "' accepted!", Toast.LENGTH_SHORT).show();
                    ((Registrator) context).setVisibilities(true);
                }
                else {
                    Toast.makeText(context, "It appears that '" + placeName + "' is already registered in our database", Toast.LENGTH_SHORT).show();
                    ((Registrator) context).goBack();
                }
            }
        }.execute();
    }
    ...
    /**
     * An iterface providing CallBack availability to the Business Registration Activity. The method provides a feedback-based
     * layout repositioning.
     */
    public interface Registrator{
        void setVisibilities(boolean post);
        void goBack();
